...
HTTP 402 and x402 are not the same thing. `402 Payment Required` is only a status
...
Two requests, a few seconds apart. No signup, no subscription, no invoice, no prior contact
between the parties. A machine buys one small thing from a stranger.
...
A seller reads a settled transfer back with two calls — `moi.InteractionByHash` for the submitted
operation (asset id, callsite, POLO-encoded beneficiary and amount) and `moi.InteractionReceipt`
for its status. Verification is a handful of chain reads, with no third party involved.

### Two things are missing

**No CAIP-2 identifier.** About 50 chains are registered with the Chain Agnostic Standards Alliance; MOI
...
**No way to ask a node which network it is.** MOI exposes 42 RPC methods across `moi.*`,
...
CAIP-2 matters beyond x402. Wallets, block explorers and cross-chain tooling all key off it.
...
MOI exposes no chain id over the wire (js-moi-utils carries an unused `Chain` enum — 111/112/113 —
but no RPC method or interaction field reads or emits it), and there is no single genesis block to
hash, so the first four rows are out today. The protocol team's pending chain-id RPC may change
this; the reference decision should wait for it.
...
Seventeen of these already exist in the same directory, including six for chains with no
...
Python only `evm`, `svm` and `tvm`. Nine of the twelve TypeScript mechanisms have no counterpart
...
The main gain is a transaction that cannot happen today.
...
After, that path exists. Settlement is the same MAS0 transfer that already works; what was
missing was an agreed way to request a payment and prove one.
