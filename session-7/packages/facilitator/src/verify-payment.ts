...
export async function runVerification(args: {
  ...
  // Participant identifiers are DERIVED from public keys, so this proves the signer really
  // controls the account being debited. Without it, a valid signature could still merely CLAIM
  // `from: <someone else>`. V1 has no sub-accounts, so this is exact equality.
  const derived = identifierFromPublicKey(inner.publicKey);
  const keyOk = derived === auth.from.toLowerCase();
  if (!add("key_binds_to_account", keyOk,
    keyOk ? `publicKey derives to ${auth.from}`
  ...
  if (registry && requirements.extra.payToAgentId) {
