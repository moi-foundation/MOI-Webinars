...
export const ACTORS = {
  OWNER: { color: magenta, role: "owns the storage" },
  AGENT: { color: cyan, role: "wants to write" },
...
function pause(): void {
  ...
  // Synchronous on purpose: banner() is called from non-async paths, and a recording aid must not
  // change the order anything happens in.
