...
import { isMock, mockChain, type MockAgentProfile } from "./mock-chain.js";

export type { AgentProfile, AgentCardInput, AgentProtocolSpec };

/** Either a real on-chain profile or the mock-mode equivalent. Same fields we actually use. */
export type Profile = AgentProfile | MockAgentProfile;
 ...
 */
/**
 * In mock mode there is no contract to talk to, so this returns null and every wrapper below
 * falls through to the in-memory registry. Callers already handle a null registry (it is also
 * what happens on devnet when the registry is unreachable), so no call site changes.
 */
...
export async function getProfile(
  ...
  if (isMock()) return mockChain.profile(agentId);
  if (!reg) return null;
...
export async function updateAgentWallet(
  ...
  if (isMock()) { mockChain.updateAgentWallet(agentId, wallet); return; }
  if (!reg) throw new Error("registry unavailable");
  await reg.updateAgentWallet(agentId, wallet);
}

/** Register an agent. In mock mode this writes to the in-memory registry. */
export async function createAgentEntry(
  reg: AgentRegistry | null,
  owner: string,
  agentWallet: string,
  info: { agentId?: string; name: string; description: string; url: string; skill: { id: string; name: string; description: string; tags: string[] } },
): Promise<string> {
  ...
  if (isMock()) {
    const agentId = info.agentId ?? `mock-agent-${mockChain.allProfiles().length + 1}`;
    const cardUri = await dataUriUploader(buildCardJson({ protocol: "a2a", protocolVersion: "1.0" }, cardInput));
    mockChain.register({
      agent_id: agentId, owner, agent_wallet: agentWallet,
      status: "ACTIVE", url: info.url, card_uri: cardUri,
    });
    return agentId;
  }
  if (!reg) throw new Error("registry unavailable");
...
export async function discoverBySkill(
  ...
  if (isMock()) {
    return mockChain.allProfiles().filter((p) => hasTag(readInlineCard(p.card_uri), tag));
  }
  if (!reg) return [];
