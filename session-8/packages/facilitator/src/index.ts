...
export async function startFacilitator(): Promise<FacilitatorHandle> {
  // The facilitator reads the registry, and registry reads require an account that EXISTS on
  // chain (see SDK_NOTES). We use the primary for that reason — it is funded.
  warnMockOnce();
  // The facilitator only READS. Registry reads build a sender and therefore need an account that
  // exists on chain, so it uses the funded primary — see SDK_NOTES.
  const facilitator = await primaryAccount();
  ...
  if (isMock()) {
    ok("in-memory registry — identity checks ACTIVE (mock)");
  } else if (config.sellerAgentId) {
  ...
  app.post("/verify", async (req, res) => {
    try {
      const outcome = await handle("POST /verify — 9 read-only checks", "verify", false, req.body);
  ...
  app.post("/settle", async (req, res) => {
    try {
      ...
      const outcome = await handle(
        "POST /settle — confirming the buyer's own transfer", "settle", true, req.body,
      );
      if (!outcome.isValid) {
