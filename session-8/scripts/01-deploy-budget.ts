...
async function main(): Promise<void> {
  ...
  if (config.logicIdOrNull) {
    ...
    summary("Budget logic ready", [["LOGIC_ID", config.logicIdOrNull], ["next", "npm run setup:agents"]]);
    return;
  }
  ...
  summary("Budget logic ready", [["LOGIC_ID", logic_id], ["next", "npm run setup:agents"]]);
}
