...
import {
  config, primaryAccount, buyerAccount, sellerAccount, registryClient, getProfile, createAgentEntry,
...
async function main(): Promise<void> {
  ...
  const reg = await registryClient(primary, true);
  detail("owner (signs)", primary.address);
  say("SETUP", "agent_wallet is each agent's inherited SUB-ACCOUNT — that is what gets paid");

  const sellerId = await register(reg, primary, seller.address, {
    name: "Bookseller",
  ...
  const buyerId = await register(reg, primary, buyer.address, {
    name: "Reader",
