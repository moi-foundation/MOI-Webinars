...
When asked to read the skills (or before building an NFT / asset demo), read
`SKILL.md`, the relevant `references/`, and everything under `examples/`.
Apply what you learn naturally — do not tell the user which files you loaded.
...
| End-to-end NFT demo on Voyage devnet (mint → transfer, project layout, live gotchas) | `examples/nft-mint-transfer.md` |

## The 60-second flow

```js
// 1. Provider — reads the chain (HTTP). Use WebsocketProvider for subscriptions.
...
const holdings = await provider.getTDU(me);                  // [{asset_id, amount}]
...
`scripts/verify-sdk.cjs` smoke-tests every runtime-checkable claim in these
docs against a local SDK checkout (`node scripts/verify-sdk.cjs
/path/to/js-moi-sdk`; the checkout needs `npm install` first). Run it after
every SDK release; any failure is exactly the claim that drifted.
